...
write_manual_recovery_wrapper() {
  download_asset "${FLASH_SCRIPT_SOURCE_BASE_URL}/scripts/flash/needle_mod.sh" "${MANUAL_RECOVERY_PATH}"
  chmod 0755 "${MANUAL_RECOVERY_PATH}"
  ln -sf "${MANUAL_RECOVERY_PATH}" /usr/local/sbin/recover-e3372h-needle
  ln -sf "${MANUAL_RECOVERY_PATH}" /usr/local/sbin/recover-e3372h-clean
}

write_set_ip_script() {
  download_asset "${FLASH_SCRIPT_SOURCE_BASE_URL}/scripts/flash/hilink_set_ip.sh" "${MODEM_IP_SCRIPT_PATH}"
  chmod 0755 "${MODEM_IP_SCRIPT_PATH}"
}
...
install_manual_recovery() {
  ...
  mkdir -p "${recovery_dir}" "${tools_dir}" "$(dirname "${MANUAL_RECOVERY_PATH}")" "$(dirname "${MODEM_IP_SCRIPT_PATH}")"

  log_info "Installing needle recovery helper"
...
setup_flash() {
  ...
  download_asset_with_fallback "Update_WEBUI_17.100.13.01.03_HILINK_Mod1.13_E3372h_local-upd.bin" "${images_dir}/Update_WEBUI_17.100.13.01.03_HILINK_Mod1.13_E3372h_local-upd.bin"
  download_asset_with_fallback "WEBUI_17.100.05.06.965_Mod1.16_V7R11_CPIO.bin" "${images_dir}/WEBUI_17.100.05.06.965_Mod1.16_V7R11_CPIO.bin"

  chmod 0755 "${tools_dir}/balong-usbload" "${tools_dir}/balong_flash_recover" || true
